...
- `deferred`: the stream is inventoried, but v0.6.0 publishes no admitted output contract.
- `intentionally-unsupported`: v0.6.0 deliberately rejects an unsafe or misleading representation.

Only rotation rate may use generic SampledData in v0.6.0, and only when its complete three-axis values prove one exact uniform period.
Accelerometer batch identity, high-frequency heart-rate confidence, and ECG guidance/per-voltage flags are not representable by bare SampledData without loss, so those streams remain native Recording Documents.
Irregular, incomplete, mixed-unit, opaque, or unreviewed streams likewise remain native rather than being resampled or relabeled.
