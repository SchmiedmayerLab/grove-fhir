...
- `deferred`: the stream is inventoried, but v0.5.0 publishes no admitted output
  contract.
- `intentionally-unsupported`: v0.5.0 deliberately rejects an unsafe or misleading
  representation.

Only rotation rate may use generic SampledData in v0.5.0, and only when its complete
