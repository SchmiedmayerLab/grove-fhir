...
| `SRSensor.acousticSettings` | `acoustic-settings` | stable-addition | 26.0 | `deferred` | — | — | Version 0.5.0 publishes no reviewed output contract for this stable platform symbol. |
| `SRSensor.ambientLightSensor` | `ambient-light` | catalog-baseline | 14.0 | `mapped-standard` | deferred | sensorkit-recording-document | The source contains heterogeneous light fields for which version 0.5.0 does not publish a lossless component profile. |
...
| `SRSensor.faceMetrics` | `face-metrics` | catalog-baseline | 17.0 | `mapped-standard` | deferred | sensorkit-recording-document | Version 0.5.0 publishes no reviewed structured profile for the heterogeneous face metrics. |
...
| `SRSensor.mediaEvents` | `media-events` | catalog-baseline | 16.4 | `mapped-standard` | deferred | sensorkit-recording-document | Media interactions do not have a reviewed source-neutral clinical Observation representation in version 0.5.0. |
...
| `SRSensor.siriSpeechMetrics` | `siri-speech-metrics` | catalog-baseline | 15.0 | `mapped-standard` | deferred | sensorkit-recording-document | Speech metrics have no reviewed source-neutral clinical Observation representation in version 0.5.0. |
| `SRSensor.sleepSessions` | `sleep-sessions` | stable-addition | 26.0 | `platform-exclusive` | sensorkit-sleep-session-observation | — | — |
| `SRSensor.telephonySpeechMetrics` | `telephony-speech-metrics` | catalog-baseline | 15.0 | `mapped-standard` | deferred | sensorkit-recording-document | Speech metrics have no reviewed source-neutral clinical Observation representation in version 0.5.0. |
| `SRSensor.visits` | `visits` | catalog-baseline | 14.0 | `platform-exclusive` | sensorkit-visit-observation | sensorkit-recording-document | — |
| `SRSensor.wristTemperature` | `wrist-temperature` | catalog-baseline | 17.0 | `platform-exclusive` | sensorkit-wrist-temperature-observation | sensorkit-recording-document | A sleep-interval wrist skin reading is neither body nor basal body temperature, so version 0.5.0 publishes a platform-scoped summary rather than binding it to a shared clinical meaning. |
