...
| VF-38 artifact-surface freshness depended on ignored build state | **Closed.** A clean checkout validates the checked-in allowlist against every tracked FSH declaration, explicit Instance usage, package identity, canonical, ordering, FHIR id, and unique resource identity. Every guide build separately compares the allowlist with that runner's exact SUSHI index and generated resources before its output can be reused. | hermetic and generated allowlist modes, build entry point, isolated clean/generated regressions |
| VF-39 schema mutation tests persisted arbitrary fixtures in clear text | **Closed.** The JSON Schema runner accepts one instance over standard input, so mutation fixtures remain in the test process pipeline and are not written to a temporary file. File-based validation remains unchanged for repository catalogs. | schema runner standard-input mode, catalog schema mutation suite, CodeQL gate |
