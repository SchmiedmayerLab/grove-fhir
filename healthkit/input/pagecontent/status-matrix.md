...
| `HKDataTypeIdentifierHeartbeatSeries` | Heartbeat Series | `deferred` | — | — | The beat-to-beat interval series has a published healthkit-heartbeat-series column schema, but the HealthKit adapter has no recording-document profile to carry it yet, so no output is admitted. |
