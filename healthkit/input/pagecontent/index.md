...
Its [JSON representation](Bundle-HealthKitStudyBundleExample.json) is a complete fixture for human inspection; it is not an operational exchange unit.
...
- [HealthKit Observation](StructureDefinition-healthkit-observation.html) requires typed deployment-scoped v2 source-record and source-output HMAC identifiers.
...
[Terminology provenance](terminology-provenance.html) records the SDK baseline, selection method, source-file hashes, ownership, and publication scope for the HealthKit names retained by version 0.6.0.
